    ...
    if " - " in constraint:
        ...
        if len(parts) == 2:
            ...
            if "." not in max_ver:
                ...
                return (
                    version_compare(version, min_ver) >= 0
                    and parse_version(version)[0] <= max_major
                )
            else:
                # Full version range
                return (
                    version_compare(version, min_ver)
                    >= 0
                    >= version_compare(version, max_ver)
                )
...
def get_latest_in_major_version(package_name: str, major_version: int) -> Optional[str]:
    ...
    try:
        ...
        major_versions = [
            v for v in versions if re.match(rf"^{major_version}\.\d+\.\d+$", v)
        ]
...
def find_updated_cdn_url(
    package_name: str, version: str, current_url: str
) -> Optional[str]:
    """
...
def extract_version_from_url(url: str, package_name: str) -> Optional[str]:
    ...
    patterns = [
        ...
        rf"bootstrap(?:cdn)?/(?:bootstrap/)?(\d+\.\d+\.\d+)",  # Bootstrap specific
        rf"datatables\.net/(\d+\.\d+\.\d+)",  # DataTables specific
    ]
...
def check_compatibility(
    ...
    for peer_pkg, peer_constraint in peer_deps.items():
        ...
        for current_pkg in all_current_versions.keys():
            if (
                current_pkg.lower() == peer_pkg_lower
                or peer_pkg_lower in current_pkg.lower()
            ):
        ...
        other_latest_version = all_latest_versions.get(
            matching_pkg, all_current_versions[matching_pkg]
        )
        ...
        if not check_semver_constraint(other_latest_version, peer_constraint):
            ...
            return {
                ...
                "suggested_version": compatible
                if compatible and compatible != "unknown"
                else None,
            }
    ...
    for other_pkg in all_current_versions.keys():
        ...
        other_latest_version = all_latest_versions.get(
            other_pkg, all_current_versions[other_pkg]
        )
...
def compare_versions(
    current: Dict[str, str], cdn_urls: Dict[str, List[str]]
) -> Tuple[
    ...
    for package, current_version in current.items():
        ...
        compat_issue = check_compatibility(
            package, latest_version, current, latest_versions
        )
        ...
        if compat_issue:
            ...
            if latest_major > current_major:
                ...
                if (
                    same_major_latest
                    and version_compare(same_major_latest, current_version) > 0
                ):
                    # Try same-major update
                    compat_issue_same_major = check_compatibility(
                        package, same_major_latest, current, latest_versions
                    )
            ...
            if (
                suggested
                and suggested != "unknown"
                and version_compare(suggested, current_version) > 0
            ):
    ...
    for package, current_version in current.items():
        ...
        if latest_version and latest_version != current_version:
            if target_version == current_version:
                ...
                conflict = (
                    compat_issue["conflicting_package"] if compat_issue else "unknown"
                )
                conflict_version = (
                    compat_issue.get("conflicting_version", "unknown")
                    if compat_issue
                    else "unknown"
                )
                print(
                ...
                if target_version == latest_version:
                    print(
                        f"📦  {package}: {current_version} → {target_version} ✅ (latest)"
                    )
                else:
                    current_major = parse_version(current_version)[0]
...
def generate_report(
    ...
    if incompatible_updates:
        ...
        for package, current, latest, cdn_urls, compat_info in incompatible_updates:
            ...
            report += f"| `{package}` | {current} ✅ | ~~{latest}~~ | Incompatible with `{conflict}` {conflict_version} |\n"
    ...
    if safe_updates:
        ...
        for package, current, target, latest, cdn_urls in safe_updates:
            if target == latest:
                ...
                report += (
                    f"| `{package}` | {current} | **{target}** | ~~{latest}~~ ⚠️ |\n"
                )
